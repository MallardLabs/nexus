...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setLabel(
          userData?.is_verified ? "Link Different Account" : "Generate Code"
        )
    ...
    try {
      ...
      if (!user) {
        embed.setDescription(
          "No account found. Generate a verification code to get started!"
        );
      } else {
        embed.addFields(
          {
            name: "Discord Account",
            value: `<@${discordId}> (${user.discord_username})`,
            inline: true,
          },
          {
            name: "Status",
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("generate_code")
          .setLabel(
            user?.is_verified ? "Link Different Account" : "Generate Code"
          )
          .setStyle(ButtonStyle.Primary)
          .setEmoji("🔗"),
        new ButtonBuilder()
  ...
  async generateNewCode(interaction) {
    try {
      ...
      // Tunda respons sementara kode sedang dibuat
      ...
      const embed = new EmbedBuilder()
        ...
        .setImage("attachment://verification.png") // Tampilkan gambar
        .setColor(COLORS.PRIMARY)
        .setFooter({ text: "☁️ Nexus runs securely on Mallard Cloud" });
      ...
      // Update respons dengan embed, tombol, dan gambar
      ...
      // Tangani error dan beri tahu pengguna
      if (interaction.isRepliable()) {
        await interaction.editReply({
          content:
            "An error occurred while generating your code. Please try again later.",
          ephemeral: true,
        });
      }
    }
    ...
    try {
      ...
      if (!existingUsers || existingUsers.length === 0) {
        ...
        if (!newUsers || newUsers.length === 0) {
          throw new Error(
            "User creation appeared to succeed but user not found"
          );
        }
